...
function validate(input) {
  ...
  if (input.name && !regexName.test(input.name)) errors.name = "Can't include special characters";
  
  if (input.name.length > 20) errors.name = "Max 20 characters";
  
  if (input.name.length < 2) errors.name = "Min 2 characters";

  if(isNaN(input.price)) errors.price = "Has to be a integer or decimal";
...
export const CreateProducts = () => {
  ...
  function handleSubmit(e) {
    ...
    else alert("The form has errors!")
      ...
      <Form className={styles.ContainerAll} onSubmit={(e) => handleSubmit(e)}>
        ...
        <Form.Group className={styles.pack} controlId="formBasicEmail">
          ...
          <Form.Control
            ...
            required={true}
          />
          {errors.price &&(<p className={styles.spanError}>{errors.price}</p>)}
        </Form.Group>
        ...
        <p className={styles.spanError}>{errors.button}</p>
      </Form>
    </div>
  );
};